...
const Result = () => {
    ...
    const fetchApi = async () => {
        try {
            const resp =
            await axios.get(`https://api.coindesk.com/v1/bpi/historical/close.json?currency=THB&start=${dateStart}&end=${dateEnd}`)
            let newArr = []
    ...
    const render = () => {
        if (loading)
            ...
            return (
                <div className='text-center space-y-3'>
                    <p className='text-2xl font-semibold'>Historical price</p>
                    <p className='text-2xl font-semibold'>(From {dateStart} To {dateEnd})</p>
